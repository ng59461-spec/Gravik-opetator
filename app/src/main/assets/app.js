...
function render(route){
 ...
 if(route.startsWith('brand:')){state.brand=decodeURIComponent(route.slice(6));if(!state.type){state.type=Object.keys(TYPES).find(t=>TYPES[t].includes(state.brand))||'Equipo';}let models=MODELS[state.brand]||['Seleccionar modelo exacto'];screen.innerHTML=head(state.brand,'Biblioteca organizada por modelo.','Equipos › '+esc(state.type||'')+' <span>›</span> '+esc(state.brand))+models.map(m=>list('model:'+encodeURIComponent(m),'M',m,'Abrir información por sistemas')).join('')+sourceRule();bind();return}
 if(route.startsWith('model:')){state.model=decodeURIComponent(route.slice(6));if(!state.brand){state.brand=Object.keys(MODELS).find(b=>(MODELS[b]||[]).includes(state.model))||'Marca';}if(!state.type){state.type=Object.keys(TYPES).find(t=>TYPES[t].includes(state.brand))||'Equipo';}screen.innerHTML=head(state.model,'Selecciona el área técnica que necesitas.',''+esc(state.brand||'Marca')+' <span>›</span> '+esc(state.model))+SYSTEMS.map((s,i)=>list('system:'+i,'0'+(i+1),s[0],s[1])).join('')+sourceRule();bind();return}
