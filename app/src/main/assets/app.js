...
function render(route){
 if(route==='inicio'){screen.innerHTML=`<section class="hero"><div><div class="eyebrow">CONOCIMIENTO QUE MUEVE GIGANTES</div><h1>Tu apoyo técnico en terreno</h1><p>Información clara, navegación rápida y orientación segura para operadores de maquinaria pesada.</p><div class="search"><input id="globalSearch" placeholder="Buscar marca, equipo o sistema..."><button class="primary" id="searchBtn">Buscar</button></div></div></section><div class="status">● Disponible sin conexión <span>V2 en desarrollo</span></div><section class="grid">${card('equipos','▣','Equipos','Tipos, marcas y modelos')}${card('simbolos','⚠','Símbolos','Alertas e indicadores')}${card('mecanica','⚙','Mecánica','Sistemas principales')}${card('diagnostico','⌕','Diagnóstico','Orientación paso a paso')}${card('checklist','✓','Checklist','Inspección preoperacional')}${card('nelson','✦','Nelson Responde','Asistente técnico GRAVIK','feature')}</section>`;bind();document.getElementById('searchBtn').onclick=search;document.getElementById('globalSearch').onkeydown=e=>{if(e.key==='Enter')search()};return}
